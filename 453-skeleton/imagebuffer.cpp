...
bool ImageBuffer::SaveToFile(const string &imageFileName)
{
    ...
    for (int y = 0; y < m_height; ++y)
        for (int x = 0; x < m_width; ++x)
        {
            ...
            float gamma = 1.0f / 1.3f;

            // gamma correction
            glm::vec3 corrected;
            corrected.r = pow(glm::clamp(color.r, 0.f, 1.f), gamma);
            corrected.g = pow(glm::clamp(color.g, 0.f, 1.f), gamma);
            corrected.b = pow(glm::clamp(color.b, 0.f, 1.f), gamma);

            pixels[i]     = (unsigned char)(255 * corrected.r);
            pixels[i + 1] = (unsigned char)(255 * corrected.g);
            pixels[i + 2] = (unsigned char)(255 * corrected.b);
        }
